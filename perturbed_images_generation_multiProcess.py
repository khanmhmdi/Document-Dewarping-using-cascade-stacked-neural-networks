...
class perturbed(utils.BasePerturbed):
    ...
    def save_img(self, m, n, fold_curve='fold', repeat_time=4, fiducial_points=16,
                 relativeShift_position='relativeShift_v2'):
        ...
        self.synthesis_perturbed_img = np.full((enlarge_img_shrink[0], enlarge_img_shrink[1], 3), 256,
        ...
        origin_pixel_position = np.argwhere(np.zeros(mesh_shape, dtype=np.uint32) == 0).reshape(mesh_shape[0],
                                                                                                mesh_shape[1], 2)
        pixel_position = np.argwhere(np.zeros(self.new_shape, dtype=np.uint32) == 0).reshape(self.new_shape[0],
                                                                                             self.new_shape[1], 2)
        ...
        x_min_per, y_min_per, x_max_per, y_max_per = self.adjust_position(perspective_shreshold, perspective_shreshold,
                                                                          self.new_shape[0] - perspective_shreshold,
                                                                          self.new_shape[1] - perspective_shreshold)
        pts1 = np.float32(
        ...
        vtx, wts = self.interp_weights(self.perturbed_xy_.reshape(self.new_shape[0] * self.new_shape[1], 2),
                                       flat_position)
        ...
        foreORbackground_label.reshape(self.new_shape[0] * self.new_shape[1], 1)[wts_sum <= 1, :] = self.interpolate(
            foreORbackground_label_map.reshape(self.new_shape[0] * self.new_shape[1], 1), vtx, wts)
        ...
        '''clip'''
        perturbed_x_min, perturbed_y_min, perturbed_x_max, perturbed_y_max = -1, -1, self.new_shape[0], self.new_shape[
            1]
        ...
        if perturbed_x_min == 0 or perturbed_x_max == self.new_shape[0] or perturbed_y_min == self.new_shape[
            1] or perturbed_y_max == self.new_shape[1]:
        ...
        if perturbed_x_max - perturbed_x_min > save_img_shape[0] or perturbed_y_max - perturbed_y_min > save_img_shape[
            ...
            synthesis_perturbed_img = cv2.resize(
                self.synthesis_perturbed_img[perturbed_x_min:perturbed_x_max, perturbed_y_min:perturbed_y_max,
                :].copy(), (im_ud, im_lr), interpolation=cv2.INTER_LINEAR)
            synthesis_perturbed_label = cv2.resize(
                self.synthesis_perturbed_label[perturbed_x_min:perturbed_x_max, perturbed_y_min:perturbed_y_max,
                :].copy(), (im_ud, im_lr), interpolation=cv2.INTER_LINEAR)
            foreORbackground_label = cv2.resize(
                self.foreORbackground_label[perturbed_x_min:perturbed_x_max, perturbed_y_min:perturbed_y_max].copy(),
            ...
            perturbed_x_min, perturbed_y_min, perturbed_x_max, perturbed_y_max = self.adjust_position_v2(0, 0, im_lr,
                                                                                                         im_ud,
                                                                                                         self.new_shape)
            ...
            self.foreORbackground_label[perturbed_x_min:perturbed_x_max,
        ...
        perturbed_x_max = self.new_shape[0] - perturbed_x_ // 2 if perturbed_x_ % 2 == 0 else self.new_shape[0] - (
                perturbed_x_ // 2 + 1)

        perturbed_y_ = max(self.new_shape[1] - (perturbed_y_max - perturbed_y_min), 0)
        perturbed_y_min = perturbed_y_ // 2
        perturbed_y_max = self.new_shape[1] - perturbed_y_ // 2 if perturbed_y_ % 2 == 0 else self.new_shape[1] - (
                perturbed_y_ // 2 + 1)
        ...
        reduce_value_x = int(round(
            min((random.random() / 2) * (self.new_shape[0] - (perturbed_x_max - perturbed_x_min)),
                min(reduce_value, reduce_value_v2))))
        reduce_value_y = int(round(
            min((random.random() / 2) * (self.new_shape[1] - (perturbed_y_max - perturbed_y_min)),
                min(reduce_value, reduce_value_v2))))
        ...
        if im_lr >= im_ud:
            self.synthesis_perturbed_color[:, perturbed_y_min:perturbed_y_max, :] = synthesis_perturbed_color[:,
        ...
        # self.synthesis_perturbed_grey[:, perturbed_y_min:perturbed_y_max, :] = synthesis_perturbed_grey[:, perturbed_y_min:perturbed_y_max, :]
            ...
            self.synthesis_perturbed_color[perturbed_x_min:perturbed_x_max, :, :] = synthesis_perturbed_color[
        ...
        if self.is_perform(0.1, 0.9):
            ...
            if self.is_perform(0.5, 0.5):
                self.synthesis_perturbed_color[:, :, :3][self.synthesis_perturbed_color[:, :, 5] == 1] = \
                    synthesis_perturbed_img_filter[self.synthesis_perturbed_color[:, :, 5] == 1]
            else:
        ...
        for l in fiducial_points_coordinate.astype(np.int64).reshape(-1, 2):
            cv2.circle(fiducial_points_synthesis_perturbed_img,
                       (l[0] + math.ceil(stepSize / 2), l[1] + math.ceil(stepSize / 2)), 2, (0, 0, 255), -1)
        cv2.imwrite(self.save_path + 'fiducial_points/' + perfix_ + '_' + fold_curve + '_' +
                    str.split(self.path, '/')[8].split('.')[0] + '.png',
                    fiducial_points_synthesis_perturbed_img)

        cv2.imwrite(
            self.save_path + 'png/' + perfix_ + '_' + fold_curve + '_' + str.split(self.path, '/')[8].split('.')[
                0] + '.png',
        ...
        forward_position = (self.synthesis_perturbed_color[:, :, 3:5] + pixel_position)[
                           self.synthesis_perturbed_color[:, :, 5] != 0, :]
        ...
        wts = wts[wts_sum <= 1, :]
        vtx = vtx[wts_sum <= 1, :]
        flat_position_forward = flat_position.reshape(save_img_shape[0], save_img_shape[1], 2)[
                                self.synthesis_perturbed_color[:, :, 5] != 0, :]
        forward_mapping.reshape(save_img_shape[0] * save_img_shape[1], 2)[wts_sum <= 1, :] = self.interpolate(
            flat_position_forward, vtx, wts)
        forward_mapping = forward_mapping.reshape(save_img_shape[0], save_img_shape[1], 2)

        mapping_x_min_, mapping_y_min_, mapping_x_max_, mapping_y_max_ = self.adjust_position_v2(0, 0, im_lr, im_ud,
                                                                                                 self.new_shape)
        ...
        self.forward_mapping[mapping_x_min:mapping_x_max, mapping_y_min:mapping_y_max] = forward_mapping[
                                                                                         mapping_x_min:mapping_x_max,
                                                                                         mapping_y_min:mapping_y_max]
        ...
        with open(self.save_path + 'grey/' + perfix_ + '_' + fold_curve + '.gw', 'wb') as f:
            ...
            f.write(pickle_perturbed_data)
        cv2.imwrite(self.save_path + 'grey_im/' + perfix_ + '_' + fold_curve + '.png',
                    self.synthesis_perturbed_color[:, :, :1])
...
def multiThread(m, n, img_path_, bg_path_, save_path, save_suffix):
    ...
    curve = threading.Thread(target=saveCurve.save_img, args=(m, n, 'curve', repeat_time, 'relativeShift_v2'),
                             name='curve')
...
def xgw(args):
        ...
        for n in range(args.sys_num):
            ...
            for m_n in range(10):
                try:
                    ...
                    saveFold.save_img(m, n, 'fold', repeat_time, fiducial_points, 'relativeShift_v2')

                    repeat_time = min(max(round(np.random.normal(8, 4)), 1), 13)
                    saveCurve.save_img(m, n, 'curve', repeat_time, fiducial_points, 'relativeShift_v2')
    ...
    #
    #                 repeat_time = min(max(round(np.random.normal(12, 4)), 1), 18)
    #                 process_pool.apply_async(func=saveFold.save_img,
    #                                          args=(m, n, 'fold', repeat_time, fiducial_points, 'relativeShift_v2'))
    #
    #                 repeat_time = min(max(round(np.random.normal(8, 4)), 1), 13)
    #                 process_pool.apply_async(func=saveCurve.save_img,
    #                                          args=(m, n, 'curve', repeat_time, fiducial_points, 'relativeShift_v2'))
    #
    #             except BaseException as err:
    #                 print(err)
    #                 continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path',
                        default='/home/mkhanmhmdi/Downloads/ImageProcessing/Synthesize-Distorted-Image-and-Its-Control-Points/scan/new/',
                        type=str,
                        help='the path of origin img.')
    parser.add_argument('--bg_path',
                        default='/home/mkhanmhmdi/Downloads/ImageProcessing/Synthesize-Distorted-Image-and-Its-Control-Points/background/',
                        type=str,
                        help='the path of bg img.')

    parser.add_argument('--output_path',
                        default='/home/mkhanmhmdi/Downloads/ImageProcessing/Synthesize-Distorted-Image-and-Its-Control-Points/Output/',
                        type=str,
                        help='the path of origin img.')
    # parser.set_defaults(output_path='test')
    parser.add_argument('--count_from', '-p', default=10, type=int,
                        metavar='N', help='print frequency (default: 10)')  # print frequency

    parser.add_argument('--repeat_T', default=5, type=int)

    parser.add_argument('--sys_num', default=10, type=int)
